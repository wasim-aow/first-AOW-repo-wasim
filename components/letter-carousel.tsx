...
export default function LetterCarousel() {
    ...
    return (
        <div className="relative w-full md:h-[800px] h-[400px] overflow-hidden bg-white ">
            {slides.map((slide, index) => (
                <div
                    key={index}
                    className={`absolute inset-0 w-full h-full transition-all duration-500 ease-in-out ${index === currentSlide
                ...
                >
                    ...
                    <div className="absolute inset-0 flex flex-col justify-center items-center text-center z-10 px-6">
                        <h3 className="text-3xl md:text-5xl font-extrabold bg-gradient-to-r from-[#121FCF] via-gray-400 to-[#B9CF2D] bg-clip-text text-transparent drop-shadow-lg">
                            {slide.title}
                        </h3>
                    </div>
                </div>
